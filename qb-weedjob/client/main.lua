...
CreateThread(function()
    ...
    --[[local WeedBlip = AddBlipForCoord(Config['delivery'].outsideLocation.x, Config['delivery'].outsideLocation.y, Config['delivery'].outsideLocation.z)
    SetBlipSprite(WeedBlip, 365) --name of the blip as well as the sprite itself more blip sprites here https://docs.fivem.net/docs/game-references/blips/
    SetBlipColour(WeedBlip, 2) --the color obviusly also on the bottom of the page https://docs.fivem.net/docs/game-references/blips/
    SetBlipScale(WeedBlip, 0.8) --scale of the blip 
    SetBlipAsShortRange(WeedBlip, true) -- This can be used to set a blip (map marker) for the job you are making
    BeginTextCommandSetBlipName("STRING") --keep this
    AddTextComponentString("Weedfarm") --name of the blip
    EndTextCommandSetBlipName(WeedBlip)]]

    while true do
        Wait(0)
        local pos = GetEntityCoords(PlayerPedId(), true)
        --uncomment if you want to use the teleporter
        --[[
        ...
        if #(pos - vector3(1060.23, -3182.07, -39.16)) < 15 and not IsPedInAnyVehicle(PlayerPedId(), false) and carryPackage == nil then
            DrawMarker(2, 1060.23, -3182.07, -39.16, 0.9, 0, 0, 0, 0, 0, 0.2001, 0.2001, 0.2001, 255, 255, 255, 255, 0, 0, 0, 0)
            if #(pos - vector3(1060.23, -3182.07, -39.16)) < 1.3 then
                if onDuty then
                    DrawText3D(1060.23, -3182.07, -39.16, "~g~E~w~ - Clock Out")
                else
                    DrawText3D(1060.23, -3182.07, -39.16, "~g~E~w~ -  Clock In")
                end
                if IsControlJustReleased(0, 38) then
                    onDuty = not onDuty
                    if onDuty then
            ...
            if packagePos ~= nil then
                ...
                if carryPackage == nil then
                    if #(pos - vector3(packagePos.x, packagePos.y, packagePos.z)) < 2.3 then
                        DrawText3D(packagePos.x,packagePos.y,packagePos.z+ 1, "~g~E~w~ - Pick Weed")
                        if IsControlJustReleased(0, 38) then
                            QBCore.Functions.Progressbar("pickup_reycle_package", "Harvesting Weed ..", math.random(4000, 6000), false, true, {
                    ...
                    if #(pos - vector3(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z)) < 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            DropPackage()
                            ScrapAnim()
                            QBCore.Functions.Progressbar("deliver_reycle_package", "Packing the Weed", 5000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                StopAnimTask(PlayerPedId(), "mp_car_bomb", "car_bomb_mechanic", 1.0)
                                TriggerServerEvent('qb-weedjob:server:getItem')
                                GetRandomPackage()
                            end)
                        end
                    else
